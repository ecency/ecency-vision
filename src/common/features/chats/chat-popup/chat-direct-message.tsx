...
export function ChatDirectMessage({ username, userClicked, lastMessage }: Props) {
  return (
    <div className="flex items-center gap-3 px-3 border-b border-[--border-color] py-2 cursor-pointer hover:bg-gray-200 dark:hover:bg-gray-800">
      <Link to={`/@${username}`}>
        <UserAvatar username={username} size="medium" />
      </Link>

      <div onClick={() => userClicked(username)}>
