...
import { getDirectMessages, GLOBAL_CHAT, notEmpty } from "./chat-utils";
import * as ls from "../util/local-storage";
// import { addDirectContacts } from "../store/chat";
...
class Raven extends TypedEventEmitter<RavenEvents, EventHandlerMap> {
  ...
  private async init() {
    ...
    this.fetchP(filters).then((resp) => {
      console.log("Init called", resp);
      // console.log("ddd", addDirectContacts);
      ...
      console.log(profile, "profile in init");
      if (profile) this.pushToEventBuffer(profile, "init");
      // const muteList = events.find((x) => x.kind.toString() === NewKinds.MuteList.toString());
      // if (muteList) this.pushToEventBuffer(muteList, "init");
      // for (const e of events.filter((x) =>
      //   [Kind.ChannelHideMessage, Kind.ChannelMuteUser].includes(x.kind)
      // )) {
      //   this.pushToEventBuffer(e, "init");
      // }
      // const channels = uniq(
      //   events
      //     .map((x) => {
      ...
      // ls.set("direct_contacts", directContacts);

      // addDirectContacts([
      //   "0be16d2d36570b96080c0f9c351feb6be515a182b8b75b90738ae0180c27c6d5",
      //   "1331c42d27e2a043ea5f478301a41ff4bab6c70bbb73f4edbf82a6756a7f51dd"
      // ]);
      ...
      // let directMessages = getDirectMessages(events as DirectMessage[],)
    });
  }
  ...
  private fetch(filters: Filter[], unsub: boolean = true) {
    console.log("Fetch Calledd", filters);

    const sub = this.pool.sub(this.readRelays, filters);

    sub.on("event", (event: Event) => {
      this.pushToEventBuffer(event, "fetch");
    });
  ...
  //   const authors = Array.from(new Set(pubs)).filter((p) => this.nameCache[p] === undefined);
  ...
  //   const batchSize = 20; // Number of authors to process in each batch
  ...
  listenMessages = (ids: string[]) => {
    // console.log("Listen messages called");
  ...
  public async sendDirectMessage(toPubkey: string, message: string, parent?: string) {
    ...
    // this.emit(RavenEvents.DirectMessage, [
    //   {
    //     id: "7a6c0db898ee043045f21fa49c5fd4b0765b506d84cb8d71539ba4cb37292040",
    //     content: "HYYYYYyyyy",
    //     peer: "1331c42d27e2a043ea5f478301a41ff4bab6c70bbb73f4edbf82a6756a7f51dd",
  ...
  //   const content = await (this.priv === "nip07"
  //     ? window.nostr!.nip04.encrypt(this.pub, JSON.stringify(list))
  //     : nip04.encrypt(this.priv, this.pub, JSON.stringify(list)));
    ...
    return new Promise((resolve, reject) => {
      this.signEvent({
        ...
        .then((event) => {
          ...
          pub.on("ok", () => {
            console.log("My event is okkkkkkkkkkkkkkkkkkkkkkk", event);
            resolve(event);
          });
  ...
  pushToEventBuffer(event: Event, from: string) {
    console.log("Push to event buffer called frommmmmmmmmm", from);
