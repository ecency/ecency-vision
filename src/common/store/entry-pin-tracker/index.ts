...
export default (
  state: EntryPinTracker = initialState,
  action: Actions,
): EntryPinTracker => {
  switch (action.type) {
...
export const trackEntryPin =
  (entry: Entry) => (dispatch: Dispatch, getState: () => AppState) => {
...
export const setEntryPin =
  (entry: Entry, pin: boolean) => (dispatch: Dispatch) => {
