...
export const sortDiscussion =
  (order: SortOrder) => (dispatch: Dispatch, getState: () => AppState) => {
    const {discussion} = getState();
...
export const updateReply =
  (reply: Entry) => (dispatch: Dispatch, getState: () => AppState) => {
    const {discussion} = getState();
