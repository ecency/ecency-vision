...
class EntryPage extends BaseComponent<Props, State> {
  ...
  detect = () => {
    ...
    const infoCard: HTMLElement | null = document.getElementById(
  ...
  ensureEntry = async () => {
    const {match, addEntry, updateEntry, addCommunity, activeUser, history} =
      this.props;
    const entry = this.getEntry();
  ...
  getEntry = (): Entry | undefined => {
    ...
    for (const k of groupKeys) {
      entry = entries[k].entries.find(
        x => x.author === author && x.permlink === permlink,
      );
      if (entry) {
        if (
          dmca.some((rx: string) =>
            new RegExp(rx).test(`${entry?.author}/${entry?.permlink}`),
          )
        ) {
          entry.body =
  ...
  render() {
                  ...
                  <div className='p-0 p-lg-5 the-entry'>
                    ...
                    <div
                      dangerouslySetInnerHTML={{
                        __html: renderPostBody(deletedEntry!.body),
                      }}
                    />
                    {editHistory && (
                      <EditHistory
    ...
    const metaProps = {
      ...
      description: `${truncate(postBodySummary(entry.body, 210), 140)} by @${
        entry.author
      }`,
      url: entry.url,
      canonical: url,
    ...
    return (
      <>
                ...
                return (
                  <>
                    ...
                    {!global.isMobile && (
                      <div id='avatar-fixed-container' className='invisible'>
                        {showProfileBox && (
                          <AuthorInfoCard {...this.props} entry={entry} />
                        )}
                      </div>
                    )}
        ...
        {editHistory && (
          <EditHistory entry={entry} onHide={this.toggleEditHistory} />
        )}
