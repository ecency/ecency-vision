...
class WitnessesPage extends BaseComponent<PageProps, State> {
  ...
  getWitness = async (witnessArray: WitnessTransformed[]) => {
    const witnessUserNamesArray: string[] = witnessArray.map(
      (item: WitnessTransformed) => {
        return item.name;
      },
    );
    try {
        ...
        (item: WitnessTransformed, index: number) => {
          try {
            const parsedArray = JSON.parse(
              accounts[index].posting_json_metadata
                ? accounts[index].posting_json_metadata
                : '',
            );
            return {
  ...
  render() {
    ...
    const header = (
      <div className='page-header mt-5'>
        ...
        {activeUser && (
          <Tsx
            k='witnesses.remaining'
            args={{n: 30 - witnessVotes.length, max: 30}}
          >
