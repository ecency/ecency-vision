...
class SubmitPage extends BaseComponent<Props, State> {
    ...
    if (activeUser?.username !== prevProps.activeUser?.username) {
      ...
      if (activeUser) {
        ...
        if (beneficiaries.find((x) => x.account === activeUser.username)) {
          const b = [
            ...beneficiaries.filter((x) => x.account !== activeUser.username),
          ];
  ...
  detectEntry = async () => {
    ...
    if (this.isEntry()) {
      ...
      this.stateSet(
        { title, tags, body, editingEntry: entry },
        this.updatePreview,
      );
    } else {
  ...
  detectDraft = async () => {
    ...
    if (this.isDraft()) {
      ...
      if (drafts.length === 1) {
        ...
        this.stateSet(
          { title, tags, body, editingDraft: draft },
          this.updatePreview,
        );
      } else {
  ...
  saveAdvanced = (): void => {
    const { reward, beneficiaries, schedule, reblogSwitch, description } =
      this.state;
  ...
  scheduleChanged = (d: Moment | null) => {
    this.stateSet(
      { schedule: d ? d.toISOString(true) : null },
      this.saveAdvanced,
    );
  };
  ...
  updatePreview = (): void => {
    ...
    this._updateTimer = setTimeout(() => {
      ...
      this.stateSet({
        preview: { title, tags, body, description },
  ...
  publish = async (): Promise<void> => {
    ...
    const summary =
      description.trim() === ''
        ? postBodySummary(this.state.body, 200)
        : description;
  ...
  saveDraft = () => {
    ...
    if (editingDraft) {
      ...
      promise = addDraft(activeUser?.username!, title, body, tagJ).then(
        (resp) => {
  ...
  render() {
    ...
    // const summary = postBodySummary(body, 200);
    //  Meta config
    const metaProps = {
    ...
    return (
      <>
        ...
        <div
          className={_c(
            `app-content submit-page ${
              editingEntry !== null ? 'editing' : ''
            } ${containerClasses}`,
          )}
        >
