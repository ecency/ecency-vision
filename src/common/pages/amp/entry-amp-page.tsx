...
class EntryPage extends BaseComponent<Props, State> {
  ...
  loadDeletedEntry = (author: string, permlink: string) => {
    commentHistory(author, permlink)
      .then(resp => {
        this.stateSet({
          deletedEntry: { body: resp.list[0].body, title: resp.list[0].title, tags: resp.list[0].tags },
          loading: false
        });
      })
      .catch(() => {
            ...
            <div className="row">
              ...
              <div className="col-12 col-lg-9">
                <div className="p-0 p-lg-5 the-entry">
                  <div
                    className="p-3 bg-danger rounded text-white my-0 mb-4 my-lg-5">{_t('entry.deleted-content-warning')}<u
                    onClick={this.toggleEditHistory}
                    className="text-primary pointer">{_t('points.history')}</u> {_t('g.logs')}.
                  </div>
