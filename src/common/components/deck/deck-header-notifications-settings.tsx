...
export const DeckHeaderNotificationsSettings = ({ title, username, setDeckDataFilters, fetchDeckData, deck }: Props) => {
    ...
    <div className="d-flex align-items-center w-100 pb-2">
      ...
      <Form.Control as="select" value={deck.dataFilters.type} onChange={typeChanged} size="sm">
        ...
        {Object.values(NotificationFilter).map(filter =>
          <option key={filter} value={filter}>{_t(`notifications.type-${filter}`)}</option>)}
      </Form.Control>
    </div>
