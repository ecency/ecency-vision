...
export default class NotificationHandler extends Component<Props> {
  componentDidMount() {
    ...
    const {activeUser, notifications, fetchUnreadNotificationCount} =
      this.props;
  ...
  nwsConnect = () => {
    ...
    window.nws = new WebSocket(
      `${defaults.nwsServer}/ws?user=${activeUser.username}`,
    );
  ...
  render() {
    const notificationSound = this.props.global.isElectron
