...
export default ({ match, history, setStepOne, setStepTwo, step }: Props) => {
  ...
  return (
    <div className="sticky-container" id="sticky-container">
      ...
      <div
        className={`mb-4 sm:mb-5 flex items-center bg-light-200 dark:bg-dark-200 md:hidden h-[64px] sticky top-0 ${
          step === 1 ? "transparent" : ""
        }`}
      >
        <div className="h-[40px] w-[40px] shrink-0 absolute l-[10px] t-[10px] cursor-pointer">
          {activeUser !== null ? (
            ...
            <img src={logo} className="h-[40px] w-[40px]" alt="Logo" onClick={handleIconClick} />
          )}
        </div>
      ...
      {!smVisible && (
        <div
          className={`mb-4 select-none relative py-3 ${
      ...
      <div
        ref={navRef}
        className={_c(
          `mb-4 select-none relative py-3 ${
            !transparentVerify && step === 1 ? "transparent" : ""
