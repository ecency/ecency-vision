...
export class ImageUpload extends BaseComponent<Props, State> {
    ...
    render() {
        ...
        return <div className="image-upload-dialog-content">
            <Form.Group>
                <Form.Label>{title}</Form.Label>
                <InputGroup className="mb-3">
                    <Form.Control type="text" disabled={inProgress} placeholder="https://" value={image} maxLength={500} onChange={this.imageChanged}/>
                    <InputGroup.Append>
...
export default class ImageUploadDialog extends Component<Props> {
    render() {
        ...
        return (
            <Modal animation={false} show={true} centered={true} onHide={onHide} keyboard={false} className="image-upload-dialog modal-thin-header">
