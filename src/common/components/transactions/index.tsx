        ...
        <>
          {hbd_payout.amount > 0 && (
            <span className='number'>
              {formattedNumber(hbd_payout.amount, {suffix: 'HBD'})}
            </span>
          )}
    ...
    if (tr.type === 'claim_reward_balance') {
      ...
      numbers = (
        <>
          {reward_hbd.amount > 0 && (
            <span className='number'>
              {formattedNumber(reward_hbd.amount, {suffix: 'HBD'})}
            </span>
          )}
    ...
    if (tr.type === 'comment_reward') {
      ...
      numbers = (
        <>
          {payout.amount > 0 && (
            <span className='number'>
              {formattedNumber(payout.amount, {suffix: 'HBD'})}
            </span>
          )}
        </>
      );
    ...
    if (tr.type === 'collateralized_convert') {
      ...
      numbers = (
        <>
          {amount.amount > 0 && (
            <span className='number'>
              {formattedNumber(amount.amount, {suffix: 'HIVE'})}
            </span>
          )}
        </>
      );
  ...
  componentDidUpdate(prevProps: any) {
    ...
    if (prevProps.transactions.list !== transactions.list) {
      const new_transactions = [
        ...(prevProps.transactions.group === transactions.group
          ? transactionsList
          : []),
        ...transactions.list,
      ];
