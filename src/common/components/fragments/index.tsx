    ...
    render() {
        ...
        return <div className="">
            <Form ref={this.form} onSubmit={(e: React.FormEvent) => {
                ...
                addFragment(activeUser.username, title, body).then(() => {
                    onAdd()
                }).catch(() => {
...
export class EditFragment extends BaseComponent<EditProps, EditState> {
    ...
    delete = () => {
        ...
        this.stateSet({inProgress: true})
        deleteFragment(activeUser.username, item.id).then(() => {
            onUpdate();
        }).catch(() => {
            error(_t('g.server-error'));
        }).finally(() => {
    ...
    render() {
        ...
        return <>
            <Form ref={this.form} onSubmit={(e: React.FormEvent) => {
                ...
                const {activeUser, item, onUpdate} = this.props;
                this.stateSet({inProgress: true})
                updateFragment(activeUser.username, item.id, title, body).then(() => {
                    onUpdate();
                }).catch(() => {
                    error(_t('g.server-error'));
                }).finally(() => {
                ...
                <div className="d-flex justify-content-between">
                    <div>
                        <PopoverConfirm onConfirm={() => {
                            ...
                            <Button variant="outline-danger" type="button" disabled={inProgress} style={{marginRight: "6px"}}>
                                {_t("g.delete")}
                            </Button>
                        </PopoverConfirm>
                        <Button variant="outline-primary" type="button" disabled={inProgress} onClick={this.back}>
...
export class Fragments extends BaseComponent<Props, State> {
    ...
    fetch = () => {
        ...
        getFragments(activeUser?.username!).then(items => {
