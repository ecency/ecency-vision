...
export class Boost extends BaseComponent<Props, State> {
    ...
    init = () => {
        ...
        return getBoostOptions(activeUser.username).then(r => {
    ...
    checkBalance = () => {
        ...
        const balanceError = parseFloat(activeUser.points.points) < amount ? _t('trx-common.insufficient-funds') : "";
    ...
    render() {
        ...
        return <div className="boost-dialog-content">
            {step === 1 && (
                <div className={`transaction-form ${inProgress ? 'in-progress' : ''}`}>
                    ...
                    <div className="transaction-form-body">
                        ...
                        <Form.Group as={Row}>
                            <Form.Label column={true} sm="2">{_t('boost.amount')}</Form.Label>
                            <Col sm="10">
                                <div className="slider-area">
                                    <div className="slider-price">
                                        {formattedNumber(this.pointsToSbd(amount), {fractionDigits: 3, suffix: '$'})}
...
export default class BoostDialog extends Component<Props> {
    render() {
        const {onHide} = this.props;
        return (
            <Modal animation={false} show={true} centered={true} onHide={onHide} keyboard={false} className="boost-dialog modal-thin-header" size="lg">
