...
export class BookmarkBtn extends BaseComponent<Props> {
    ...
    render() {
        ...
        if (bookmarkId) {
            return (
                <div className={_c(`bookmark-btn bookmarked ${inProgress ? "in-progress" : ""}`)} onClick={this.delete}>
                    <Tooltip content={_t('bookmark-btn.delete')}><span>{bookmarkSvg}</span></Tooltip>
                </div>
            );
        }
