...
export class BookmarkBtn extends BaseComponent<Props> {
  ...
  componentDidUpdate(prevProps: Readonly<Props>) {
    ...
    if (
      ...
      // or entry changed
      !(
        entry.author === prevProps.entry.author &&
        entry.permlink === prevProps.entry.permlink
      )
    ) {
      this.detect();
    }
  ...
  render() {
    ...
    if (bookmarkId) {
      return (
        <div
          className={_c(
            `bookmark-btn bookmarked ${inProgress ? 'in-progress' : ''}`,
          )}
          onClick={this.delete}
        >
    ...
    return (
      <div
        className={_c(`bookmark-btn ${inProgress ? 'in-progress' : ''}`)}
        onClick={this.add}
      >
        <Tooltip content={_t('bookmark-btn.add')}>
          <span>{bookmarkOutlineSvg}</span>
