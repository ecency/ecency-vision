...
export class EntryIndexMenu extends Component<Props, States> {
  constructor(props: Props) {
    ...
    if (
      ...
      pathname.includes(activeUser.username)
    ) {
      isGlobal = false;
    }
    let showInitialIntroductionJourney =
      activeUser &&
      isActiveUser(activeUser) &&
      ls.get(`${activeUser.username}HadTutorial`);
    if (
      activeUser &&
      isActiveUser(activeUser) &&
      (showInitialIntroductionJourney === "false" ||
        showInitialIntroductionJourney === null)
    ) {
      ...
      if (typeof window !== "undefined") {
        document.getElementById("overlay") &&
          document
            ...
            .classList.remove("overlay-for-introduction");
        ...
        let entryIndexMenuElements = document.getElementsByClassName(
          "entry-index-menu"
        );
  ...
  onChangeGlobal() {
    ...
    if (history.location.pathname.includes("/my")) {
      ...
      filter !== "feed" &&
        history.push("/" + filter + (tag.length > 0 ? "" : "/my"));
    }
  }
  ...
  componentDidUpdate(prevProps: Props) {
    ...
    if (
      history.location.pathname.includes("/my") &&
      !isActiveUser(activeUser)
    ) {
      history.push(history.location.pathname.replace("/my", ""));
    } else if (
  ...
  onPreviousWeb = () => {
    ...
    switch (value) {
      ...
      case IntroductionType.TRENDING:
        value =
          activeUser && isActiveUser(activeUser)
            ? IntroductionType.FRIENDS
            : IntroductionType.NONE;
        break;
      default:
  ...
  render() {
    ...
    const menuConfig: {
      ...
      items: [
        ...[EntryFilter.trending, EntryFilter.hot, EntryFilter.created].map(
          (x) => {
            return {
    ...
    const introductionDescription = (
      <>
        ...
        {introduction === IntroductionType.FRIENDS &&
          _t("entry-filter.filter-global-part4")}
        {introduction === IntroductionType.FRIENDS && (
          <Link to="/discover">
            {" "}
    ...
    const introductionOverlayClass =
      (isMounted &&
        (introduction === IntroductionType.NONE
          ? "d-none"
          : "overlay-for-introduction")) ||
      "d-none";
    return isMounted ? (
      <div>
        <div
          className={introductionOverlayClass}
          id="overlay"
                  ...
                  <ul className={`nav nav-pills nav-fill`}>
                    {menuConfig.items.map((i, k) => {
                      return (
                        <li
                          key={k}
                          className={`nav-item ${i.flash ? "flash" : ""}`}
                        >
                          <Link
                            to={i.href!}
                            className={_c(
