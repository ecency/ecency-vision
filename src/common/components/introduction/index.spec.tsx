...
describe('Introduction component', () => {
  const props = {
    ...
    media:
      'https://propakistani.pk/wp-content/uploads/2019/12/Social-Media-Insight-top.png',
    onClose: () => {},
  };
  ...
  it('(2) has title, media and description', () => {
    expect(component.root.findByProps({id: 'title'}).children).toContain(
      props.title,
    );
    expect(component.root.findByProps({id: 'media'}).props.src).toContain(
      props.media,
    );
    expect(component.root.findByProps({id: 'description'}).children).toContain(
      props.description,
    );
  });
