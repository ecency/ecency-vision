...
export class WithdrawRoutes extends BaseComponent<Props, State> {
    ...
    sign = (key: PrivateKey) => {
        ...
        setWithdrawVestingRoute(activeUser.username, key, account, Number(percent) * 100, auto === "yes")
    ...
    render() {
        ...
        if (mode === "sign") {
            return <>
                ...
                <p className="text-center"><a onClick={(e) => {
                    e.preventDefault();
                    this.stateSet({mode: ""});
        ...
        return <>
            <Form ref={this.form} onSubmit={(e: React.FormEvent) => {
                ...
                <Form.Row>
                    ...
                    <Form.Group as={Col} md={2} className="d-md-flex align-items-end justify-content-center">
                        <Button type="submit" size="sm">{_t("g.add")}</Button>
                    </Form.Group>
                </Form.Row>
            </Form>
...
export default class WithdrawRoutesDialog extends Component<Props> {
    render() {
        ...
        return (
            <Modal animation={false} show={true} centered={true} onHide={onHide} keyboard={false} className="withdraw-routes-dialog">
