...
export default class SuggestionList extends Component<Props> {
  ...
  getPossibleInput = (): HTMLInputElement | null => {
    return this.parent.current?.querySelector(
      "input[type='text']",
    ) as HTMLInputElement;
  };
  ...
  focusItem = (index: number) => {
    const el = this.parent.current?.querySelectorAll('.list-item')[
      index
    ] as HTMLLinkElement;
    if (el) {
      el.focus();
  ...
  getFocusedIndex = (): number => {
    const el = this.parent.current?.querySelector(
      '.list-item:focus',
    ) as HTMLLinkElement;
  ...
  render() {
    const {children, items, header, renderer, containerClassName, modeItems} =
      this.props;
    const {showList} = this.state;
          ...
          return (
            _items.length > 0 && (
              <div className='suggestion-list' key={modeKey}>
                {modeItem.header && (
                  <div className='list-header'>{modeItem.header}</div>
                )}
                <div className='list-body'>
                  {_items.map((x: any, i: number) => {
                    const content = modeItem.renderer
                      ? modeItem.renderer(x)
                      : x;
                    return (
                      <a
