...
it("(1) Sort trending", () => {
  ...
  sortDiscussions(replies, SortOrder.trending);
...
it("(2) Sort author_reputation", () => {
  const [, ...replies] = discussionInstace1;
  sortDiscussions(replies, SortOrder.author_reputation);
...
it("(3) Sort author_repuvotestation", () => {
  const [, ...replies] = discussionInstace1;
  sortDiscussions(replies, SortOrder.votes);
...
it("(4) Sort created", () => {
  const [, ...replies] = discussionInstace1;
  sortDiscussions(replies, SortOrder.created);
