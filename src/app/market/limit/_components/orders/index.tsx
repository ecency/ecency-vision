...
const buyColumns = [`${i18next.t("market.price")}`, `HIVE`, `${i18next.t("market.total")} HBD ($)`];
...
export const Orders = ({ type, loading, data, onPriceClick }: Props) => {
  ...
  let title = `${i18next.t("market.buy")} ${i18next.t("market.orders")}`;
  ...
  switch (type) {
    ...
    case 2:
      columns = buyColumns;
      title = `${i18next.t("market.sell")} ${i18next.t("market.orders")}`;
      mappedData = (data as OrdersDataItem[]).map((item: OrdersDataItem) => {
        return {
          key3: (item as OrdersDataItem).hbd / 1000,
    ...
    case 3:
      ...
      title = `${i18next.t("market.trade-history")}`;
