  ...
  addressKeyup = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.keyCode === 13) {
      ...
      if (pathMatch) {
        ...
        let validAddress =
          url.pathname[0] == '/' ? url.pathname.replace('/', '') : url.pathname;
        this.setState({address: validAddress});
...
export class NavControls extends Component<NavControlsProps> {
  ...
  render() {
    ...
    const reloadClassName = _c(
      `reload ${!reloadFn || reloading ? 'disabled' : ''}`,
    );
...
export class NavBar extends Component<Props, State> {
  ...
  render() {
    ...
    const logoHref = activeUser
      ? (match && re.test(match.params.name)) ||
        (global.tag === `@${activeUser.username}` && global.filter !== 'feed')
        ? '/hot'
        : global.filter === 'feed'
    ...
    const textMenuConfig = {
      ...
      postElem: (
        <div className='drop-down-menu-version'>Ecency Surfer {version}</div>
      ),
    };
    ...
    return (
      <>
        ...
        <div
          ref={this.nav}
          className={_c(`nav-bar-electron ${noMargin ? 'mb-0' : ''}`)}
        >
        ...
        {global.newVersion && (
          <Updater
            global={global}
            dismissNewVersion={this.props.dismissNewVersion}
          />
        )}
      </>
    );
